...
| 실제 설치 (Claude Code) | 로컬 마켓플레이스 등록·플러그인 설치 완료. `claude plugin details`로 네 스킬(`brainstorm`/`spec`/`writing-plan`/`develop`) 발견 확인 |
...
로컬 경로로 직접 등록해 실제 설치·발견까지 확인한 절차와 결과는 [Claude 설치 검증 기록](docs/validation/claude-installation-validation.md)에 있다.
