...
| figures | writing-plan은 그림별 소속·연결 위치·설명할 것·핵심 메시지. develop은 승인된 핵심에 의도·내용·간단한 스케치 구성·실제 캡션·각주·HTML 본문 위치 연결을 구체화. 실제 그림은 생성하지 않음 |
...
- [x] **Step 1: 통합 시나리오의 실제 입력과 판정 기준을 추가한다.**
...
- [x] **Step 2: 통합 시나리오를 실행하고 실제 결정을 판정한다.**
...
- [x] **Step 4: 패키지만 다른 위치로 복사해 상대 참조와 호출 정책을 확인한다.**
...
- [x] **Step 5: runtime 지침의 완결성과 참조 범위를 점검한다.**
...
- [x] **Step 6: README와 상태를 실제 근거에 맞춰 갱신한다.**
...
- [x] **Step 7: 마지막 검증과 커밋 후 결과를 인계한다.**
